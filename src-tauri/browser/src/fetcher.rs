...
impl Fetcher {
    ...
    fn download(&self, revision: &str) -> Result<PathBuf> {
        ...
        info!("{}", &format!("安装包路径: {}", path.display()));
        ...
        info!("{}", &format!("全部下载完成"));
        info!("{}", &format!("文件下载到: {}", path.display()));
        Ok(path)
    }
    ...
    fn unzip<P: AsRef<Path>>(zip_path: P) -> Result<PathBuf> {
        ...
        info!("{}", &format!("解压中, 需要一点时间: {}", extract_path.display()));
